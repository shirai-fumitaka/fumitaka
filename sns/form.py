...
class FriendsForm(forms.Form):
    def __init__(self,user,*args, **kwargs):
        ...
        self.fields['friends'] = forms.MultipleChoiceField(
            choices=[(item.user,item.user)for item in Friend.objects.filter(owner=user)],
...
class PostForm(forms.Form):
    content = forms.CharField(max_length=100,widget=forms.Textarea(attrs={'class':'form-control'}) )
